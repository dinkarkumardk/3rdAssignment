...
| Analytics access | Python scripts only | Enough for demo | Expose the Databricks Data Warehouse for BI + API use |
...
flowchart TB
	...
	DataWarehouseNode[[Data Warehouse — Governed Delta tables]]
...
The flow is intentionally linear: data lands on S3, Spark-based cleaning notebooks enforce quality gates, ETL notebooks shape trusted Delta tables inside the Data Warehouse, and every consumer—dashboards, APIs, NLP, and ML—pulls from that governed layer while Redis accelerates repeat lookups.
...
flowchart TB
	...
	Curated[[Curated Delta tables — Data Warehouse]]
...
4. **Analytics and ML share code:** The same Databricks workspace maintains the Data Warehouse, serves dashboards, and runs model training, so there’s no second engine to maintain.
...
flowchart TB
	...
	subgraph Pipeline
		...
		Ingest --> Landing[S3 landing - raw Parquet/Delta]
		Landing --> Cleaning[Cleaning & Great Expectations]
		Cleaning --> ETL[ETL & Feature build]
	ETL --> DW[Data Warehouse - modeled Delta tables]
	end

	subgraph Consumption
		direction TB
		DW --> Dash[Dashboards & APIs]
		DW --> NLP[NLP service]
...
3. **ETL & feature modeling** – The same serverless Spark jobs build fact/dimension tables plus the unified analytics dataset, then load them into the governed Data Warehouse. This step mirrors `create_unified_dataset`, but scales and can be re-run incrementally.
...
5. **Expose the Data Warehouse + connect NLP app** – replace pandas reads with data warehouse queries returning only the necessary slices (reduces data transfer and lets multiple clients share governed access).
...
- **Consistent analytics** – BI dashboards, APIs, and ML predictions all read from the same curated tables inside the Data Warehouse.
